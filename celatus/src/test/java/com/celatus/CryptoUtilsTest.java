...
public class CryptoUtilsTest {
    ...
    public void testKeyGeneration() { // Equal strings should return equal keys
        ...
        Assert.assertEquals(CryptoUtils.generateAESKey(password1), CryptoUtils.generateAESKey(password2));
        Assert.assertNotEquals(CryptoUtils.generateAESKey(password1), CryptoUtils.generateAESKey(password3));
    }
    ...
    @Test
    public void testDataDecryption() { // The decrypted text block should be equal to the plain text original input
        String password = "haha";
        ...
        // The same IV should be used for both encryption and decryption, but unlike the key it does not need to be kept secret
